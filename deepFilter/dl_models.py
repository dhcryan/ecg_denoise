...
def cross_domain_attention(query, key, value, head_size, num_heads, dropout=0):
    """
    Cross-Domain Attention module where query comes from one domain and key/value from another.
    """
    # Layer Normalization
    ...
    return layers.Add()([attention_output, query])

def Transformer_COMBDAE_with_CrossDomainAttention(signal_size=sigLen, head_size=64, num_heads=8, ff_dim=64, num_transformer_blocks=6, dropout=0):
    input_shape = (signal_size, 1)
    ...
    x_time = cross_domain_attention(x_time, x_freq, x_freq, head_size, num_heads, dropout)
    
    # 주파수 도메인에서 시간 도메인에 주의
    x_freq = cross_domain_attention(x_freq, x_time, x_time, head_size, num_heads, dropout)
    
    # 특징 결합
    combined = layers.Concatenate()([x_time, x_freq])
    
    # 디코더
    x5 = Conv1DTranspose(combined, filters=64, kernel_size=ks, activation='elu', strides=1, padding='same')
