...
- Loss blends MAD + SSD; may overweight spikes—consider Huber or frequency-domain consistency term for smoother recon.

## 12. Quick Troubleshooting
| Symptom | Cause | Fix |
|---------|-------|-----|
| ImportError deepFilter | Ran from wrong cwd | `cd dual_freqdae_repo` or add repo root to PYTHONPATH |
| Shape mismatch !=512 | Wrong input length | Use `--segment` or pad/crop externally |
| Slow first run | FFT dataset build | Use `--reuse-cache` on subsequent runs |
| CUDA plugin warnings | Duplicate TF init | Safe to ignore unless crash occurs |

## 13. License & Attribution
Original research code adapted from full ECG denoising repository. PhysioNet QT Database per its license. Please cite relevant ECG denoising and FAN layer publications if publishing results.

## 14. Technology Transfer Summary
See `docs/TECH_TRANSFER.md` for detailed operational guidance, onboarding, maintenance, and adaptation steps.

---
For enhancements (attention extraction, pruning, quantization) create an additional script or request a new extension.
