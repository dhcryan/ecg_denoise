...
#                         'Vanilla NL',
#                         'Multibranch LANL',
#                         'Multibranch LANLD','Transformer_DAE','Transformer_COMBDAE','Transformer_FreqDAE']
#     # Load Results DRNN
...
#     with open('1127/test_results_' + dl_experiments[8] + '.pkl', 'rb') as input:
#         test_Transformer_FreqDAE = pickle.load(input)
...
#         "IIR Filter": test_IIR,
#         "DRNN": test_DRNN,
#         "FCN_DAE": test_FCN_DAE,
#         "Vanilla_L": test_Vanilla_L,
#         "Vanilla_NL": test_Vanilla_NL,
#         "Multibranch_LANL": test_Multibranch_LANL,
#         "Multibranch_LANLD": test_Multibranch_LANLD,
#         "Transformer_DAE": test_Transformer_DAE,
#         "Transformer_COMBDAE": test_Transformer_COMBDAE,
#         "Transformer_FreqDAE": test_Transformer_FreqDAE
#     }

#     print("Calculating Multi-shot results for Transformer-COMBDAE...")
#     shot_counts = [1, 3, 5, 10]  # Multi-shot settings
#     multi_shot_results = {}

#     [X_test, y_test, _] = models["Transformer_COMBDAE"]
#     X_test, y_test = tf.convert_to_tensor(X_test, dtype=tf.float32), tf.convert_to_tensor(y_test, dtype=tf.float32)

#     for shots in shot_counts:
#         SSD_values, MAD_values, PRD_values, COS_SIM_values, SNR_values = [], [], [], [], []

#         for i in range(len(X_test)):
#             noisy_signal = tf.expand_dims(X_test[i], axis=0)  # Add batch dimension
#             clean_signal = tf.expand_dims(y_test[i], axis=0)

#             # Multi-shot denoising
#             output = 0
#             for _ in range(shots):
#                 # Assuming `model.denoising` is the denoising function
#                 output += model.denoising(noisy_signal)
#             output /= shots

#             # Convert tensors to numpy
#             output_np = output.numpy().squeeze()
#             clean_signal_np = clean_signal.numpy().squeeze()

#             # Calculate metrics
#             SSD_values.append(SSD(clean_signal_np, output_np))
#             MAD_values.append(MAD(clean_signal_np, output_np))
#             PRD_values.append(PRD(clean_signal_np, output_np))
#             COS_SIM_values.append(COS_SIM(clean_signal_np, output_np))
#             SNR_values.append(SNR(clean_signal_np, output_np))

#         # Store results for each shot
#         multi_shot_results[f"Transformer_COMBDAE_{shots}-shots"] = {
#             "SSD": np.array(SSD_values),
#             "MAD": np.array(MAD_values),
#             "PRD": np.array(PRD_values),
...
#     SSD_all, MAD_all, PRD_all, COS_SIM_all, SNR_all = [], [], [], [], []
#     Exp_names = ['FIR Filter', 'IIR Filter'] + dl_experiments


#     for model_name, data in models.items():
#         [X_test, y_test, y_pred] = data
#         SSD_all.append(SSD(y_test, y_pred))
#         MAD_all.append(MAD(y_test, y_pred))
#         PRD_all.append(PRD(y_test, y_pred))
#         COS_SIM_all.append(COS_SIM(y_test, y_pred))
#         SNR_all.append(SNR(y_test, y_pred))

#     # Add Multi-shot results
#     for shots, results in test_results_multi_shot.items():
#         SSD_all.append(results["SSD"])
#         MAD_all.append(results["MAD"])
#         PRD_all.append(results["PRD"])
#         COS_SIM_all.append(results["COS_SIM"])
#         SNR_all.append(results["SNR"])
#         Exp_names.append(shots)

#     ####### Results Visualization #######
#     metrics = ['SSD', 'MAD', 'PRD', 'COS_SIM', 'SNR']
#     metric_values = [SSD_all, MAD_all, PRD_all, COS_SIM_all, SNR_all]
...
#     # [X_test, y_test, y_pred] = test_Transformer_FreqDAE
...
#     print("Calculating Segmentation-based Metrics...")
#     segm = [0.2, 0.6, 1.0, 1.5, 2.0]  # Noise level ranges
#     rnd_test = np.load('rnd_test.npy')

#     SSD_seg_all, MAD_seg_all, PRD_seg_all, COS_SIM_seg_all, SNR_seg_all = [], [], [], [], []

#     for idx_exp in range(len(Exp_names)):
#         SSD_seg, MAD_seg, PRD_seg, COS_SIM_seg, SNR_seg = [[] for _ in range(len(segm) - 1)], \
#                                                           [[] for _ in range(len(segm) - 1)], \
#                                                           [[] for _ in range(len(segm) - 1)], \
#                                                           [[] for _ in range(len(segm) - 1)], \
#                                                           [[] for _ in range(len(segm) - 1)]
...
#                     COS_SIM_seg[idx_seg].append(COS_SIM_all[idx_exp][idx])
...
if __name__ == "__main__":
    ...
    with open('1202/test_results_' + dl_experiments[8] + '.pkl', 'rb') as input:
        test_Transformer_FreqDAE = pickle.load(input)
